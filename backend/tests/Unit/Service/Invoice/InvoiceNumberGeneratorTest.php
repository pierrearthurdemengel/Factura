<?php

namespace App\Tests\Unit\Service\Invoice;

use App\Entity\Company;
use App\Service\Invoice\InvoiceNumberGenerator;
use Doctrine\DBAL\Connection;
use Doctrine\DBAL\Result;
use Doctrine\ORM\EntityManagerInterface;
use PHPUnit\Framework\TestCase;
use Symfony\Component\Uid\Uuid;

class InvoiceNumberGeneratorTest extends TestCase
{
    /**
     * Verifie que le premier numero de l'annee est FA-AAAA-0001.
     */
    public function testGeneratesFirstNumberOfYear(): void
    {
        $companyId = Uuid::v4();
        $company = $this->createCompanyMock($companyId);

        $connection = $this->createConnectionMock([
            'last_invoice_number' => null,
            'last_invoice_year' => null,
        ]);

        $em = $this->createMock(EntityManagerInterface::class);
        $em->method('getConnection')->willReturn($connection);

        $generator = new InvoiceNumberGenerator($em);
        $number = $generator->generate($company);

        $currentYear = (int) date('Y');
        $this->assertSame(sprintf('FA-%d-0001', $currentYear), $number);
    }

    /**
     * Verifie que le numero s'incremente correctement.
     */
    public function testIncrementsNumber(): void
    {
        $companyId = Uuid::v4();
        $company = $this->createCompanyMock($companyId);
        $currentYear = (int) date('Y');

        $connection = $this->createConnectionMock([
            'last_invoice_number' => 42,
            'last_invoice_year' => $currentYear,
        ]);

        $em = $this->createMock(EntityManagerInterface::class);
        $em->method('getConnection')->willReturn($connection);

        $generator = new InvoiceNumberGenerator($em);
        $number = $generator->generate($company);

        $this->assertSame(sprintf('FA-%d-0043', $currentYear), $number);
    }

    /**
     * Verifie la reinitialisation du compteur en debut d'annee.
     */
    public function testResetsNumberOnNewYear(): void
    {
        $companyId = Uuid::v4();
        $company = $this->createCompanyMock($companyId);
        $currentYear = (int) date('Y');

        $connection = $this->createConnectionMock([
            'last_invoice_number' => 150,
            'last_invoice_year' => $currentYear - 1,
        ]);

        $em = $this->createMock(EntityManagerInterface::class);
        $em->method('getConnection')->willReturn($connection);

        $generator = new InvoiceNumberGenerator($em);
        $number = $generator->generate($company);

        $this->assertSame(sprintf('FA-%d-0001', $currentYear), $number);
    }

    private function createCompanyMock(Uuid $id): Company
    {
        $company = $this->createMock(Company::class);
        $company->method('getId')->willReturn($id);

        return $company;
    }

    /**
     * @param array<string, mixed> $row
     */
    private function createConnectionMock(array $row): Connection
    {
        $result = $this->createMock(Result::class);
        $result->method('fetchAssociative')->willReturn($row);

        $connection = $this->createMock(Connection::class);
        $connection->method('executeQuery')->willReturn($result);
        $connection->method('executeStatement')->willReturn(1);

        return $connection;
    }
}
